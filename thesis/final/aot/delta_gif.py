...
def main(prompt, outpath):
    ...
    diffs = torch.concat(pred_original_samples).diff(dim=0).absolute().mean(dim=1).cpu().numpy()
    ...
    plt.scatter(range(len(diffs)), diffs.sum(axis=(1,2)))
    plt.savefig(os.path.join(outpath, 'dot.png'))
    plt.clf()
    plt.close()

    util.to_gif(images=images, path=os.path.join(outpath, 'diff.gif'))        
