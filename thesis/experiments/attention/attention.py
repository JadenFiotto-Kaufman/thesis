...
class AttentionHookModule(torch.nn.Module):
    ...
    def forward(self, probs):
        ...
        if cross_attention:
            ...
            upsampled = F.interpolate(torch.transpose(probs, 1, 2).view(n_heads, n_dim, h, w), size=(64, 64), mode='bicubic')

        else:
        
            upsampled = F.interpolate(torch.transpose(probs, 1, 2).view(1, n_heads, n_dim, h, w), size=(4096, 64, 64), mode='trilinear')[0]
